...
public class NodeDao implements Dao<Node> {
    ...
    public NodeDao() throws SQLException {
        insertPreparedStatement = Manager.getConnection().prepareCall("INSERT INTO nodes \n"
                + " (id, lat, lon, _user, uid, visible, version, changeset, _timestamp) \n"
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        batchInsertPreparedStatement = Manager.getConnection().prepareCall("INSERT INTO nodes \n"
                + " (id, lat, lon, _user, uid, visible, version, changeset, _timestamp) \n"
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
//        node.setTimestamp();
    ...
    public void saveExecute(Node t) throws SQLException {
        String sql = String.format("INSERT INTO nodes \n"
                ...
                t.getId(), t.getLat(), t.getLon(), t.getUser().replace("'", ""),
                t.getUid(), t.isVisible(), t.getVersion(), t.getChangeset(), t.getTimestamp());
    ...
    public Optional<Node> getById(long id) throws SQLException {
        String sql = "SELECT * FROM nodes \n"
                + "WHERE id = " + id;
        try (ResultSet rs = Manager.getConnection().createStatement().executeQuery(sql)) {
            if (rs.next()) {
    ...
    public int delete(Node t) throws SQLException {
        String sql = "DELETE FROM nodes \n"
                + "WHERE id = " + t.getId();
        return Manager.getConnection().createStatement().executeUpdate(sql);
    }

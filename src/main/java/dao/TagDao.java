...
public class TagDao implements Dao<TagDB> {
    ...
    public TagDao() throws SQLException {
        this.insertPreparedStatement = Manager.getConnection().prepareCall("INSERT INTO tags"
                + "(key, value, node_id)"
                + "VALUES (?, ?, ?)");
        this.batchInsertPreparedStatement = Manager.getConnection().prepareCall("INSERT INTO tags"
                + "(key, value, node_id)"
                + "VALUES (?, ?, ?)");;
    }
    ...
    public void saveExecute(TagDB t) throws SQLException {
        String sql = String.format("INSERT INTO tags "
                + "(key, value, node_id) "
                + "VALUES ('%s', '%s', %s)", t.getKey(), t.getValue().replace("'", ""), t.getNodeId());
